...
pub fn part_b(contents: &str) -> i32 {
    ...
    let result = [(start, end), (end, start), (start, end)]
        .iter()
        .fold((0, 0), |acc, trek| {
            let start_state = ValleyState::new(trek.0, 0, acc.1);
            let (path, mins) = astar(
                &start_state,
                |p| p.neighbours(&directions, &walls, &blizzard_states, &valley_area),
                |p| manhattan(&(p.pos.0, p.pos.1), &trek.1),
                |p| p.pos == trek.1,
            )
            .unwrap();
            return (acc.0 + mins, path.last().unwrap().blizzards);
        });

    return i32::try_from(result.0).unwrap();
}
