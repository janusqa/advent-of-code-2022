...
pub fn part_b(contents: &str) -> String {
    ...
    for instruction in instructions {
        let num_items_to_move = instruction.0;
        let from_stack = stacks.get_mut(&instruction.1).unwrap();
        let to_remove = from_stack.len().saturating_sub(num_items_to_move);
        let mut crane = from_stack.drain(to_remove..).collect::<Vec<&str>>();
        stacks.get_mut(&instruction.2).unwrap().append(&mut crane);
    }
